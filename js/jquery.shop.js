(function( $ ) {
	...
	$.Shop.prototype = {
		...
		handleAddToCartForm: function() {
			...
			self.$formAddToCart.each(function() {
				...
				$form.on( "submit", function() {
					...
					var discountRates = self._calculateDiscount( qty ); 
					//calculate discount rate based on quantity of book purchased
					
					sTotal = sTotal * discountRates ; // sTotal will be updated as its original value multiplies with the discountrate computed earlier, its either 1, 0.95 or 0.85 indicated no discount, 5 percent discount and 15% discount.
					...
					var shippingRates = self._calculateShipping( sTotal );
					//calculate shipping offer based on sTotal
					
					var totalShipping = shipping + shippingRates;// RM 10 + shipping offer, either +0 or -10.	
		...
		_calculateShipping: function( sTotal ) {
			var shipping = 0;
			if( sTotal < 100 ) { //if total price is larger than 100
				shipping = +10; // then shipping fee minus RM10 
			}			
			return shipping;
		},
		
		_calculateDiscount: function( qty ) {
			var per = 0;
			if( qty < 5 ) { //if quantity of book less than 5
				per = 1; // total price is still 100% original 
			}	
			
			if( qty >= 5 && qty <=10) { //if quantity of book between 5~10
				per = 0.95; // total price is 95% of the original , means 5% discount
			}
			if( qty >10) { //if quantity of book more than 10
				per = 0.85; // total price is 85% of the original , means 15% discount
			}
			return per;
		},
